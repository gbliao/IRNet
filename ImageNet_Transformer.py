...
class ImageNet(nn.Module):
    def __init__(self, args=None):
        ...
        self.rgb_backbone = PyramidVisionTransformer(patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 18, 3], sr_ratios=[8, 4, 2, 1])
        self.fs_backbone = PyramidVisionTransformer(patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 18, 3], sr_ratios=[8, 4, 2, 1])
        self.load_pretrained_model('./pvt_medium.pth')
        ...
        cp.append(nn.Sequential(nn.Conv2d(320, 128, 3, 1, 1), self.relu, nn.Conv2d(128, k_channel, 3, 1, 1), self.relu))
        ...
        cp_fs.append(nn.Sequential(nn.Conv2d(320, 128, 3, 1, 1), self.relu, nn.Conv2d(128, k_channel, 3, 1, 1), self.relu))
    ...
    def forward(self, image_Input, FS_Input):
        ...
        for i in range(4):
            rgb_feature_extract.append(self.CP[i](tmp_rgb[i])) 
        ...
        for i in range(4):
            fs_feature_extract.append(self.CP_FS[i](tmp_FS[i])) 
        ...
        FS_fea_1_8 = FS_1_8.reshape(FS_1_8.shape[0], FS_1_8.shape[1] * FS_1_8.shape[2], FS_1_8.shape[3], FS_1_8.shape[4]).contiguous()
        FS_fea_1_16 = FS_1_16.reshape(FS_1_16.shape[0], FS_1_16.shape[1] * FS_1_16.shape[2], FS_1_16.shape[3], FS_1_16.shape[4]).contiguous()        
        FS_fea_1_32 = FS_1_32.reshape(FS_1_32.shape[0], FS_1_32.shape[1] * FS_1_32.shape[2], FS_1_32.shape[3], FS_1_32.shape[4]).contiguous()
